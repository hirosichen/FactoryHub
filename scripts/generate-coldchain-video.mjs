...
async function main() {
  console.log('Generating cold-chain-video...')
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if (d.status === 'done') {
      ...
      fs.writeFileSync(path.join(OUTPUT_DIR, 'cold-chain-video.mp4'), buf)
      console.log(`✓ cold-chain-video.mp4 (${(buf.length/1024/1024).toFixed(1)} MB)`)
      return
    }
