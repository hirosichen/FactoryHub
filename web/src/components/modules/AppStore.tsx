...
import { Search, Download, Star, Zap, Wifi, Shield, ArrowRight, CheckCircle2, Clock, Cpu } from 'lucide-react'
...
const CATEGORIES = [
  { id: 'all', label: '全部' },
  { id: 'iot', label: 'IoT 協議整合' },
  { id: 'ai', label: 'AI 智能模組' },
  { id: 'monitor', label: '監控儀表板' },
  { id: 'quality', label: '品質管理' },
  { id: 'supply', label: '供應鏈' },
...
function StatusButton({ status }: { status: AppModule['status'] }) {
  switch (status) {
    case 'installed':
      return (
        <Button variant="outline" size="sm" className="text-green-600 border-green-300 gap-1" disabled>
          <CheckCircle2 className="h-3.5 w-3.5" /> 已安裝
        </Button>
      )
    case 'available':
      return (
        <Button size="sm" className="gap-1">
          <Download className="h-3.5 w-3.5" /> 安裝
        </Button>
      )
    case 'coming-soon':
      return (
        <Button variant="secondary" size="sm" className="gap-1" disabled>
          <Clock className="h-3.5 w-3.5" /> 即將推出
        </Button>
      )
  }
}
    ...
    <div className="p-6 space-y-6 overflow-auto h-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">探索 AI 智慧模組，一鍵部署到您的工廠平台</p>
        </div>
      ...
      <Card className="border-primary/30 overflow-hidden">
        <div className="relative h-40 overflow-hidden">
          ...
          <div className="absolute top-4 left-6 flex items-center gap-2">
            <Badge className="bg-primary text-primary-foreground text-[10px]">核心能力</Badge>
            <Badge variant="outline" className="text-[10px] text-white border-white/40">最多人安裝</Badge>
          </div>
        </div>
        <CardContent className="p-6 -mt-10 relative">
          <div className="flex items-start gap-6">
            <div className="h-16 w-16 rounded-2xl bg-background shadow-lg border flex items-center justify-center text-3xl shrink-0">🔌</div>
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-bold text-foreground mb-1">AI 協議閘道器 <span className="text-sm font-normal text-muted-foreground">AI Protocol Gateway</span></h3>
          ...
          <div className="mt-4 pt-4 border-t border-primary/10">
            <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">AI 自動串接流程</p>
            <div className="flex items-center gap-2 text-xs">
              <div className="flex items-center gap-1.5 bg-background rounded-lg px-3 py-2 border">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full h-9 pl-9 pr-3 rounded-md border border-input bg-transparent text-sm shadow-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
          />
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {filtered.map((mod) => (
          <Card key={mod.id} className={cn(
            ...
            mod.status === 'coming-soon' && 'opacity-70'
          )}>
            <div className="relative h-32 overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 flex items-end justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="h-10 w-10 rounded-xl bg-background/90 backdrop-blur-sm flex items-center justify-center text-xl shadow-sm">{mod.icon}</div>
                  <div>
                    <div className="font-semibold text-white text-sm drop-shadow-sm">{mod.name}</div>
                    <div className="text-[11px] text-white/80 drop-shadow-sm">{mod.nameEn}</div>
                  </div>
                </div>
            ...
            <CardContent className="p-4 pt-3">
              ...
              {mod.aiCapability && (
                <div className="bg-primary/5 rounded-lg px-3 py-2 mb-3 flex items-start gap-2">
                  <Zap className="h-3.5 w-3.5 text-primary shrink-0 mt-0.5" />
                  <span className="text-[11px] text-foreground leading-relaxed">{mod.aiCapability}</span>
                </div>
              )}

              {mod.protocols && mod.protocols.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                  {mod.protocols.slice(0, 4).map(p => (
                    <Badge key={p} variant="outline" className="text-[10px] px-1.5 py-0">{p}</Badge>
                  ))}
                  {mod.protocols.length > 4 && (
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0">+{mod.protocols.length - 4}</Badge>
                  )}
                </div>
              )}
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t">
                ...
                <span className="flex items-center gap-1"><Download className="h-3 w-3" /> {mod.installs}</span>
